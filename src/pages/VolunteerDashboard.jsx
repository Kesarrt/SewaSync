...
import { collection, query, where, getDocs, updateDoc, doc, onSnapshot, increment } from 'firebase/firestore';
import { onAuthStateChanged } from 'firebase/auth';
import { auth, db } from '../firebase';
import { CheckCircle, MapPin, Award, User, Phone, Activity, BookOpen, AlertCircle } from 'lucide-react';
...
export default function VolunteerDashboard() {
  ...
  if (!user) {
    return <div className="min-h-screen bg-slate-50 flex items-center justify-center text-slate-500 font-medium">Please log in to view your dashboard.</div>;
  }

  if (!volunteerDoc) {
    return <div className="min-h-screen bg-slate-50 flex items-center justify-center text-slate-500 font-medium">No approved volunteer record found for {user.email}. Please apply first.</div>;
  }
    ...
    <div className="min-h-screen bg-slate-50 p-6 font-sans">
      <div className="max-w-4xl mx-auto">
        <header className="flex justify-between items-center mb-8">
          <div>
            <h1 className="text-3xl font-black text-slate-900 tracking-tight">SewaSync <span className="text-indigo-600">Volunteer</span></h1>
          ...
          <div className="bg-white px-4 py-2 rounded-2xl shadow-sm border border-slate-100 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="font-black text-slate-800 leading-none">{volunteerDoc.credits || 0}</p>
            </div>
          </div>
        </header>
        ...
        {needsProfileUpdate ? (
          ...
          <div className="bg-white rounded-[2rem] p-8 shadow-sm border border-slate-100 max-w-2xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="flex items-center gap-4 mb-8 pb-6 border-b border-slate-50">
              <div className="bg-indigo-50 p-4 rounded-2xl text-indigo-600 shadow-inner">
                <AlertCircle size={28} />
              </div>
              <div>
                <h2 className="text-xl font-black text-slate-800">Complete Your Setup</h2>
              ...
              <div className="grid grid-cols-2 gap-6">
                <div>
                  ...
                  <input 
                    ...
                    className="w-full bg-slate-50 border border-slate-200 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 rounded-xl px-4 py-3.5 outline-none transition-all font-medium text-slate-700" 
                ...
                <div>
                  ...
                  <select 
                    ...
                    className="w-full bg-slate-50 border border-slate-200 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 rounded-xl px-4 py-3.5 outline-none transition-all font-medium text-slate-700 appearance-none"
                  >
          ...
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            {tasks.length === 0 ? (
              <div className="bg-white p-12 rounded-[2rem] text-center border border-slate-100 shadow-sm">
                <div className="bg-slate-50 w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-5 text-slate-300">
                  <CheckCircle size={40} />
                </div>
              ...
              <div className="grid gap-5 md:grid-cols-2">
                {tasks.map(task => (
                  <div key={task.id} className="bg-white rounded-3xl p-6 shadow-sm border border-slate-100 hover:border-indigo-200 hover:shadow-md transition-all group relative overflow-hidden">
                    ...
                    <button 
                      ...
                      className="w-full bg-slate-50 hover:bg-emerald-500 text-slate-500 hover:text-white font-bold py-3.5 rounded-xl transition-all flex items-center justify-center gap-2 border border-slate-100 hover:border-emerald-600 group/btn"
                    >
